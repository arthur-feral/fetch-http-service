...
describe('tools', () => {
  ...
  describe('formatUrlWithQueryParameters', () => {
    it('should add ? if no query params', () => {
      expect(
        formatUrlWithQueryParameters(
          'http://www.domain.ext/endpoint/',
          {
            key: 'value',
          },
        ),
      ).to.equal(
        'http://www.domain.ext/endpoint/?key=value',
      );
    });
    ...
    it('should not add an extra ?', () => {
      expect(
        formatUrlWithQueryParameters(
          'http://www.domain.ext/endpoint/?cogip=1',
          {
            key: 'value',
          },
        ),
      ).to.equal(
        'http://www.domain.ext/endpoint/?cogip=1&key=value',
      );
    });
    ...
    it('should format an url with query params properly from an object', () => {
      expect(
        ...
        'http://www.domain.ext/endpoint/?cogip=1&ids=1%2C2%2C3&key=value&redirect=https%3A%2F%2Fwww.domain.ext%2Fendpoint%2F1%3Flol%3Dtulip%23%2Fanhash', // eslint-disable-line
      );
    });
  });
